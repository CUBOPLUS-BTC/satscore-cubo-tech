"""
app.btcprotocol.wordlist
===================
BIP-39 English mnemonic wordlist — educational reference.

Contains the complete 2048-word English wordlist specified in BIP-39
(https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt).

This module is intentionally self-contained: it does not import from
other app.btcprotocol submodules so it can be used as a dependency by bip39.py
without creating circular imports.

Public API
----------
BIP39_WORDLIST          - tuple of 2048 words (sorted, lower-case ASCII)
WORDLIST_HASH           - SHA-256 hex digest of the joined wordlist
get_word(index)         -> str
get_index(word)         -> int   (-1 if not found)
suggest_words(prefix)   -> list  (autocomplete, up to 8 suggestions)
validate_word(word)     -> bool
verify_wordlist_integrity() -> bool
"""

import hashlib

# ---------------------------------------------------------------------------
# The complete BIP-39 English wordlist (2048 words, BIP-39 specification)
# ---------------------------------------------------------------------------
...
BIP39_WORDLIST = (
    ...
    "crane", "crash", "crazy", "cream", "credit", "creek", "crew", "cricket",
    ...
    "fluid", "flush", "fly", "foam", "focus", "fog", "foil", "follow",
    ...
    "gain", "galaxy", "gallery", "game", "gap", "garbage", "garden", "garlic",
    ...
    "grasp", "grass", "gravity", "great", "green", "grid", "grief", "grit",
    "grocery", "group", "grow", "grunt", "guard", "guide", "guilt", "guitar",
    "gun", "gym", "habit", "hair", "half", "hammer", "hamster", "hand",
    "happy", "harsh", "harvest", "hat", "have", "hawk", "hazard", "head",
    ...
    "honey", "hood", "hope", "horn", "hospital", "host", "hour", "hover",
    "hub", "huge", "human", "humble", "humor", "hundred", "hungry", "hunt",
    "hurdle", "hurry", "hurt", "husband", "hybrid", "ice", "icon", "ignore",
    "ill", "illegal", "image", "imitate", "immense", "immune", "impact", "impose",
    "improve", "impulse", "inbox", "income", "increase", "index", "indicate", "indoor",
    ...
    "keen", "keep", "ketchup", "key", "kick", "kid", "kingdom", "kiss",
    "kit", "kitchen", "kite", "kitten", "kiwi", "knee", "knife", "knock",
    "know", "lab", "ladder", "lamp", "language", "laptop", "large", "later",
    ...
    "liar", "liberty", "library", "license", "life", "lift", "like", "limb",
    ...
    "lumber", "lunar", "lunch", "luxury", "mad", "magic", "magnet", "maid",
    "main", "major", "make", "mammal", "mango", "mansion", "manual", "maple",
    ...
    "minor", "minute", "miracle", "miss", "misery", "mixture", "mobile", "model",
    ...
    "neglect", "neither", "nephew", "nerve", "nest", "never", "news", "next",
    ...
    "pull", "pulp", "pulse", "pumpkin", "punish", "pupil", "purchase", "purity",
    "purpose", "push", "put", "puzzle", "pyramid", "quality", "quantum", "quarter",
    ...
    "region", "regret", "regular", "reject", "relax", "release", "rely", "remain",
    ...
    "rookie", "rose", "rotate", "rough", "round", "route", "royal", "rubber",
    ...
    "shove", "shrimp", "shrug", "shuffle", "shy", "sibling", "siege", "sight",
    "sign", "silent", "silk", "silly", "silver", "similar", "simple", "since",
    "sing", "siren", "sister", "situate", "six", "size", "sketch", "skill",
    ...
    "snow", "soap", "soccer", "social", "sock", "solar", "soldier", "solid",
    ...
    "syrup", "table", "tackle", "tag", "tail", "talent", "tank", "tape",
...
# Ensure exactly 2048 words (BIP-39 specification)
assert len(BIP39_WORDLIST) == 2048, (
    f"BIP39_WORDLIST must contain exactly 2048 words, got {len(BIP39_WORDLIST)}"
)
...
_WORD_TO_INDEX: dict = {word: i for i, word in enumerate(BIP39_WORDLIST)}

# ---------------------------------------------------------------------------
# Integrity hash
# ---------------------------------------------------------------------------

def _compute_wordlist_hash() -> str:
    """Compute SHA-256 of the joined wordlist for integrity verification."""
    joined = "\n".join(BIP39_WORDLIST)
    return hashlib.sha256(joined.encode("utf-8")).hexdigest()


WORDLIST_HASH: str = _compute_wordlist_hash()
...
def get_word(index: int) -> str:
    """Return the BIP-39 word at the given index (0-2047).

    Parameters
    ----------
    index:
        Integer in range [0, 2047].
    ...
    if not 0 <= index <= 2047:
        raise IndexError(
            f"get_word: index {index} out of range [0, 2047]"
        )
    return BIP39_WORDLIST[index]


def get_index(word: str) -> int:
    """Return the 0-based index of *word* in the BIP-39 wordlist.

    Parameters
    ----------
    word:
        Lowercase BIP-39 word.

    Returns
    -------
    Index (0-2047), or -1 if the word is not in the list.
    """
    return _WORD_TO_INDEX.get(word.lower().strip(), -1)


def suggest_words(prefix: str, max_results: int = 8) -> list:
    """Return up to *max_results* BIP-39 words that start with *prefix*.

    Useful for mnemonic autocomplete UIs.  The search is case-insensitive.
    ...
    Returns
    -------
    Sorted list of matching word strings.
    """
...
def validate_word(word: str) -> bool:
    """Return True if *word* is in the BIP-39 English wordlist.

    The comparison is case-insensitive and strips surrounding whitespace.
    """
    return word.lower().strip() in _WORD_TO_INDEX


def verify_wordlist_integrity() -> bool:
    """Return True if the current wordlist matches the stored hash.

    Recomputes the SHA-256 of the joined wordlist and compares it to
    :data:`WORDLIST_HASH`.  Can be used at startup to detect corruption.
    """
    return _compute_wordlist_hash() == WORDLIST_HASH
